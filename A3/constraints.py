...
class QueensTableConstraint(TableConstraint):
    ...
    def __init__(self, name, qi, qj, i, j):
        ...
        self.assigns = [[ci, cj] for ci in qi.domain() for cj in qj.domain()
...
class NValuesConstraint(Constraint):
    ...
    def hasSupport(self, var, val):
        ...
        if var not in self.scope():
            return True
        def double_bd(l):
            return self._lb <= sum([1 for (variable, value) in l
                                    if value in self._required]) <= self._ub
        def up_bd(l):
            return sum([1 for (variable, value) in l
                        if value in self._required]) <= self._ub
        assign = self.scope()
        assign.remove(var)
...
class allOnce(Constraint):
    ...
    def check(self):
        ...
        for var in self.scope():
            if not var.isAssigned():return True
            elif var.getValue() != 0:

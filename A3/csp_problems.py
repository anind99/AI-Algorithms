...
def planeCSP(planes_problem):
    ...
    var_array = [[Variable("{},{}".format(p[i], j), [0] + planes_problem._can_fly[p[i]])
                  for j in range(len(planes_problem._can_fly[p[i]]))] for i in range(len(p))]
    valid_connect = [[0,0]] + [list(pair) for pair in planes_problem.can_follow[:]] + [[fl, 0] for fl in f]
    vars = [var for row in var_array for var in row]
...
def formatted_sol(solutions, silent):
    solutions_formatted = []
    if len(solutions) == 0:
        if not silent: print("No solutions")
    else:
        for s in solutions:
            sol = dict()
            for (var, val) in s:
                ...
                if plane in sol:
                    ...
                    sol[plane] = [val]
            solutions_formatted.append([[key] + [n for n in sol[key] if not n == 0] for key in sol.keys()])
...
def solve_planes(planes_problem, algo, allsolns,
    ...
    '''This function takes a planes_problem (an instance of PlaneProblem
       ...
       plane.
    '''
    return formatted_sol(bt_search(algo, planeCSP(planes_problem), variableHeuristic, allsolns, trace)[0], silent)
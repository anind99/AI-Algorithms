    ...
    def chooseAction(self, gameState):
        ...
        livingGhostPositionDistributions = \
            [beliefs for i, beliefs in enumerate(self.ghostBeliefs)
             if livingGhosts[i+1]]
        "*** YOUR CODE HERE ***"
        maxB = [max(d, key=lambda x: d[x]) for d in livingGhostPositionDistributions]
        return min([(min(self.distancer.getDistance(Actions.getSuccessor(pacmanPosition,a), ghost)
                         for ghost in maxB), a) for a in legal])[1]